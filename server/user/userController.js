...
module.exports = {
    creatUser: (req, res) =>{
        ...
        const param = [req.body.name, req.body.lastname, req.body.email, req.body.password]

        // 회원가입 시 비밀번호 
        bcrypt.hash(param[3], saltRounds, (error, hash)=>{
            param[3] = hash
            console.log(param)
            mql.query('INSERT INTO users(`name`, `lastname`, `email`, `password`) VALUES (?,?,?,?)', param, (err, row) => {
    ...
    loginUser: (req, res) => {
        ...
        mql.query('SELECT * FROM users WHERE email=?', param[0], (err, row) =>{
            ...
            bcrypt.compare(param[1], row[0].password, (error, result)=>{
                ...
                mql.query('UPDATE users SET token=? WHERE email=? AND password=?', [userToken, param[0], row[0].password], (err, row)=>{
